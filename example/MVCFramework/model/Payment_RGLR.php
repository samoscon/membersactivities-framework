...
class Payment_RGLR extends \membersactivities\model\subscriptions\PaymentTypeImplementation {
    #[\Override]
    public function statusReceived(\model\Payment $payment, string $token, string $status): void {
        ...
        if($status === 'paid'){
            ...
            $download = $this->getDownloadText($payment, $token, $subscription->costitem->activity);
...
_MAIL_;
    ...
    public function getDownloadText(\model\Payment $payment, string $token, \membersactivities\model\activities\Activity $activity): string {
        if(!$activity->isComposite()) {
          if (_WALLETISSUERID) {
            ...
            return 'You can <a href="'._APPDIR.'pdfTickets?id='.($payment->getId()).'&access_token='.$token.'">'
                    . 'download your tickets in pdf format'
                    . '</a>'
                    . ' or <br><br>'
                    . '<a href="'.$googleWalletToken.'"><img src="'.$imgsrc.'" style="width:192px;height:48px;" alt="Download your tickets in Google Wallet"/></a>'
                    ;
          } else {
            return 'You can <a href="'._APPDIR.'pdfTickets?id='.($payment->getId()).'&access_token='.$token.'">'
                    . 'download your tickets in pdf format'
